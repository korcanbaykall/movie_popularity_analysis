...
**`imdb_votes_distribution.png` — Distribution of IMDb number of votes.** Histogram of the raw `numVotes` column. Unlike the rating, this is the most heavy-tailed variable in the dataset: more than 99% of films have fewer than ~100,000 votes, so the histogram is dominated by a single tall bar near zero, while a thin tail extends past 3 million. Values span roughly seven orders of magnitude on the raw scale. This is why I derived `log_numVotes = log1p(numVotes)` for section 9 — without the log transform, linear models would be dominated entirely by a handful of blockbuster titles.
...
Test A is the formal version of the project's main question and provides the statistical justification for moving to ML. Test C is interesting on its own: the correlation is statistically significant but very small (ρ = −0.14) and negative, which means IMDb rating and TMDb popularity capture different things — TMDb popularity tracks present-day audience attention more than critical quality.
...
- **Metadata-only feature set:** the same set with `averageRating` and `log_numVotes` removed. `numVotes` correlates strongly with popularity (Spearman ρ ≈ 0.62) and is basically an engagement proxy; the metadata-only set is the version that matches the proposal's framing — *can categorical metadata alone predict popularity?*
...
- **Action is a popularity-relevant genre.** The Action / non-Action median gap (0.635 vs 0.364) is both large and statistically significant.
...
1. **Metadata alone is informative but not sufficient.** Without `numVotes` and `averageRating`, the regressor still reaches R² ≈ 0.32, which is roughly five times better than the median baseline (R² = −0.07), and the classifier still reaches ROC-AUC ≈ 0.80. So categorical metadata genuinely carries predictive signal, which directly answers the project's main question.
...
**`ml_pred_vs_actual_full.png` — Predicted vs actual `log1p(popularity)`, full feature set.** Four side-by-side scatter panels (Baseline median, Linear Regression, Random Forest, Gradient Boosting), each plotting predicted values against actual values on the held-out test set with the red 45° dashed line marking perfect prediction. The Baseline panel collapses to a horizontal stripe at the training median (~0.32) — the median model has only one value to give. Linear Regression aligns roughly along the diagonal but compresses the high end, systematically under-predicting actual values above ~1.0. Random Forest and Gradient Boosting both track the diagonal more tightly through the central mass of the data, with GB the best overall (test R² = 0.594). All three trained models share one clear failure mode: the few very high-popularity points in the upper-right corner (actual log1p ≈ 1.8–2.2) get pulled toward the centre, and even the best model puts the most extreme point at only ≈ 1.0. This happens because the dataset has a heavy right tail and very few training examples that high up.

**`ml_pred_vs_actual_meta.png` — Predicted vs actual, metadata-only feature set.** The same four-panel layout, but trained without `log_numVotes` and `averageRating`. The Baseline panel is identical (it does not use features). The trained-model panels show visibly more scatter around the diagonal, more compression at the high end, and a tighter cluster of predictions near the central mode. The regression R² drops from ≈ 0.59 (full) to ≈ 0.32 (metadata-only), and this figure shows where that lost variance went: the metadata-only model can still place most points roughly in the right region of the popularity axis, but it can no longer pull the high-popularity examples up the diagonal — its predictions for actual log1p > 1.0 are barely distinguishable from its predictions for actual log1p ≈ 0.5.

**`ml_feature_importance.png` — Top 15 feature importances (Gradient Boosting).** Two horizontal bar charts side by side, one per feature set. For the **full** model, `log_numVotes` alone occupies more than 60% of the total importance, with `lang_top_ja`, `startYear`, `averageRating`, and `runtimeMinutes` trailing far behind. This puts numbers on what the EDA already implied: when an engagement signal is available, the model basically becomes a one-feature predictor. The **metadata-only** chart looks very different. No single feature dominates: `runtimeMinutes` and `startYear` lead with ~0.22 and ~0.19, then `lang_top_ja` and `genre_Documentary` cluster around 0.10, followed by a long tail of genre flags (Thriller, Action, Sci-Fi, Adventure, Fantasy, Animation, Horror) and the English-language indicator. So the metadata-only model is genuinely *combining* runtime, era, language, and genre signals rather than reading off one dominant variable. The Japanese-language indicator keeps showing up high, probably because anime and animation films are ranked high on TMDb in spring 2026.

**`ml_roc_curves.png` — ROC curves for the three classifiers.** Two panels (full vs metadata-only), each plotting the true-positive rate against the false-positive rate for Logistic Regression, Random Forest, and Gradient Boosting; the dashed diagonal is the no-skill baseline (AUC = 0.5). For the **full** model all three curves rise sharply toward the top-left, with Random Forest leading at AUC = 0.878, Gradient Boosting (0.875) almost overlapping it, and Logistic Regression slightly below (0.855). The curves stay well above 0.8 true-positive rate even at low false-positive rates, meaning the model finds most popular films while flagging relatively few non-popular ones. The **metadata-only** panel tells a softer version of the same story: Random Forest 0.801, Gradient Boosting 0.793, Logistic Regression 0.751. The vertical gap between the two panels at any fixed false-positive rate is what the "0.08 of AUC" difference tabulated above actually looks like, and that gap is widest in the low-FPR region — the region you'd care about if you wanted high-precision predictions.

**`ml_confusion_matrix.png` — Confusion matrices for the best classifier on each feature set.** Two heatmaps in shades of blue, for Random Forest with `class_weight='balanced'` (the highest-AUC model in both feature sets) on the 194-row test set. The **full-feature** matrix has 135 true negatives (Not popular → Not popular), 31 true positives (Popular → Popular), 11 false positives, and 17 false negatives. The **metadata-only** matrix degrades all four cells modestly: 128 TN, 25 TP, 18 FP, 23 FN. Two things to note. First, both models are biased toward predicting "Not popular" — expected given the 25/75 class imbalance even after balancing weights. Second, most of the lost accuracy when removing engagement features lands on the *positive* class: recall on Popular drops from 31/48 ≈ 65% (full) to 25/48 ≈ 52% (metadata-only), while specificity on Not popular only drops from 93% to 88%. The classifier struggles most exactly where the prediction task is hardest — identifying the popular minority from structural metadata alone.

---

Across these five figures the picture is the same: when an engagement signal is available, prediction is largely a one-feature problem (`log_numVotes`) and accuracy is high. When only structural metadata is available, the model has to combine many weaker signals (runtime, era, language, genre flags), and the predictive ceiling is lower but still well above baseline.
